...
wsServer.on("connection", (socket) => {
  socket.on("join_room", (data) => {
    console.log("join room 시작.");
    console.log(data, "엥");
    console.log(data.room, "dsdsd하이염");
    console.log(data.user, "유저 정보");
    if (users[data.room]) {
      ...
      users[data.room] = [
        { id: socket.id, user: data.user, userName: data.user.userName },
      ];
    }
  ...
  socket.on("participants", (roomName) => {
    ...
    console.log(users[roomName], "으아오");
    socket.emit("participants", users[roomName]);
  });
...
app.post("/api/rooms/make", (req, res) => {
  Room.findOne({ roomName: req.body.roomName }).exec((err, roomInfo) => {
    if (roomInfo)
      return res.json({
        success: false,
        exist: true,
...
app.post("/api/rooms/attBookUpdate", (req, res) => {
  Room.findOneAndUpdate(
    { _id: req.body.roomId },
    { attendanceBook: req.body.attendanceBook },
    (err, room) => {
      if (err) return res.json({ success: false, err });
...
app.post("/api/votes/voteDelete", (req, res) => {
  Vote.findOneAndDelete({ _id: req.body.voteId }).exec((err, vote) => {
    if (err) return res.json({ success: false, err });
    return res.json({ success: true });
  });
});
...
app.post("/api/votes/voteUpdate", (req, res) => {
  Vote.findOneAndUpdate(
    { _id: req.body.voteId },
    { options: req.body.options, voted: req.body.voted },
    (err, vote) => {
      if (err) return res.json({ success: false, err });
      return res.json({ success: true });
    }
